...
| `transient_skip_error_codes` | `[429, 503]` | Global default HTTP error codes that stop the provider for the current card after one key attempt (the failed key cools down, so the next card rotates to the next key). Empty list disables entirely — every key is tried within one card. |
| `transient_skip_providers` | `{}` | Per-provider override: `{provider: [codes]}`. Set from the provider ✏️ edit window (Skip on). Empty list disables the one-attempt stop for that provider; missing provider follows the global defaults. |
