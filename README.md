...
- **Incremental Fast Scan**: Re-running a batch only scans notes created since the deck's last full scan (tracked **per-deck**, including sub-decks), making repeat runs near-instant — with a **Force FULL scan** option to re-check everything.
