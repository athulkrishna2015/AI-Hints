...
### May 11, 2026
- **Correct Answer Highlighting**: Automatically identifies the correct option and highlights it in green on the back (answer side) of the card for immediate visual feedback.
- **Version-Gated Auto Regeneration**: Configure a minimum required add-on version in the settings; any cards generated by older versions of the add-on will silently auto-regenerate during your review.
- **Improved Custom Prompt Support**: The AI client now perfectly respects custom system prompts that output `correct_answer` and `distractors` natively, avoiding legacy format enforcement and hallucinated text blocks.
