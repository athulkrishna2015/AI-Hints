    ...
    def on_fallbacks_clicked(self):
        ...
        def _save_data():
            try:
                ...
                # Save model timeouts
                new_timeouts = self.fallback_dialog.get_model_timeouts()
                if hasattr(self, "parent_dialog") and hasattr(self.parent_dialog, "model_timeouts_data"):
                    self.parent_dialog.model_timeouts_data[self.provider] = new_timeouts
...
class ManageKeysDialog(QDialog):
    ...
    def _move(self, delta: int):
        ...
        def _cell(r, c):
            ...
            if c == 0:
                return item.checkState() == Qt.CheckState.Checked if item else True
            return item.text() if item else ""

        cur = [_cell(row, c) for c in [0, 1, 2]]
        other = [_cell(target, c) for c in [0, 1, 2]]
        for r in [row, target]:
            for c in [0, 1, 2]:
                if not self.table.item(r, c):
                    self.table.setItem(r, c, QTableWidgetItem())
        for r, vals in ((row, other), (target, cur)):
            for c, val in enumerate(vals):
                if c == 0:
                    self.table.item(r, c).setCheckState(Qt.CheckState.Checked if val else Qt.CheckState.Unchecked)
                else:
