...
def write_pretty_config(addon_package, config):
    """Write config using Anki's addon manager (full replace).
...
def write_pretty_config_preserve_keys(addon_package, config):
    """Write config, preserving every key that already exists on disk.

    The addon's save paths frequently write snapshots that were built from
    ``addonManager.getConfig()``, which can silently come back as pure
    config.json defaults (package-name mismatch, corrupt/transient meta.json).
    A naive full replace then wipes the user's real config — api_keys,
    custom providers, templates, tweaks — which is exactly what happened on
    2026-08-19 (batch-start autosave destroyed addon/meta.json).

    This writer is safe against that:
    - the on-disk config is the baseline; an incoming key only wins when it
      is explicitly present (absent incoming keys keep their on-disk values),
    - api_keys additionally keeps on-disk values for keys the incoming dict
      leaves empty (the UI can reconstruct api_keys from rendered rows only),
    - the previous meta.json is copied to meta.json.bak before overwriting,
    - the file is written directly from the addon's own folder, bypassing
      Anki's name-based addon lookup.
    """
    ...
    # preserved set is the fingerprint of the 2026-08-19 defaults-snapshot bug.
    ...
    try:
        ...
        logger.warning(f"AI-Hints: could not read previous meta.json while writing: {e}")
        meta = {}
    if not isinstance(meta, dict):
        meta = {}
    ...
    try:
        ...
        _log_write(addon_package, "direct-file(preserve-merge)", on_disk, merged)
    except Exception as e:
        logger.error(f"AI-Hints: direct meta.json write failed ({e}); falling back to addonManager")
        try:
            from aqt import mw

            mw.addonManager.writeConfig(addon_package, merged)
            _log_write(addon_package, "addonManager-fallback", on_disk, merged)
        except Exception as e2:
            logger.error(f"AI-Hints: fallback addonManager write also failed: {e2}")