...
## 8.2.1 (2026-09-14)
- **Skipped Cards Are Never Auto-Generated**: Pre-generation and auto-generation now respect the `_skipped` flag, so skipped cards are no longer picked for pre-gen buffers or regenerated by the current-card auto path. Only explicit **Regenerate** (the Generate button) works on a skipped card — even `auto_regenerate_all` leaves it untouched. Unskip to make it auto-eligible again.
- **Minimal Skipped Cards**: Skipped blocks are now stored as only `{"cN": {"_skipped": true}}` — the empty `hints`/`options` arrays, the stale `_src` answer snapshot, and the decorative wrapper attributes (`data-ai-hints-addon-id`, `contenteditable`, `data-show-hints`/`data-show-options`) are removed.
