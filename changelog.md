...
- **Batch Skipped Cards No Longer Loop**: `Verification Pass` now treats empty/missing-cloze cards (`is_card_skipped`) and deleted cards as done instead of `still missing`, so a `22 → 4` run no longer loops through `Pass #4…#10` with `4 cards still missing hints` until the 10-pass cap.
- **Searchable Source Deck Stays Available**: `Batch → Source Deck` `QCompleter` model is now rebuilt in `_refresh_deck_list()` after every `clear()`/`all_names()` so the live substring filter does not go stale after deck add/delete or profile switch.
- **Batch Scan Progress No Longer Spurious-Cancels**: `QProgressDialog("Cancel")` `wasCanceled()` on window-close/ESC is now disabled (`setCancelButton(None)`) during Initiate scan, preventing `Scanning canceled by user.` aborts when the dialog loses focus.
- **Batch Scan Progress Closes on Completion**: `Scanning notes 131 of 140...` now `close()`s at `100%` on both fast and fallback paths instead of lingering at `Scanning notes 140 of 140`.
...
- **Cooldown Providers Exit the Pass Instead of Spinning**: With multithreaded batching, a provider whose models are all blacklisted/on cooldown now leaves the pass when a peer is actively serving the queue, instead of endlessly re-checking the blacklist and keeping idle threads alive. A lone provider stuck entirely on cooldown waits up to a bounded stall (batch timeout + 60s) before ceding to the Verification Pass, so a dead key set can't pin the pass open.
- **Visible "Draining" Batch State**: When the queue hits "0 left" but worker threads are still finishing in-flight requests, the batch status now shows a 🧼 **Draining** notice (how many requests remain and how long the longest has run) so a batch that is winding down no longer looks stuck.
- **Source Deck Dropdown No Longer Spans the Screen**: The Batch tab's Source Deck chooser and its autocomplete popup are height-capped (16 rows / 320px), so a large deck list can't take over the whole display window; typing-based search still narrows the list normally.
...
- **Separate Enabled / Disabled Lists**: The Advanced Global Fallback Priority dialog now shows two tables side by side behind a draggable splitter in one window — **Enabled priority order** (the rows actually tried, top to bottom) and **Disabled / available**, each with its own ▲ Up / ▼ Down buttons. Every column header is draggable to resize. Unchecking a row moves it to Disabled; checking one there moves it back to Enabled (appended at the end). Group Same Models, Move, and header sorting each operate within their own list so one list's order never disturbs the other's; fetched models land in Disabled. The redundant Sort menu and Rank buttons are removed (header clicks cover sorting), and Group Same Models no longer freezes on thousand-row lists: reordering rebuilds rows in bulk instead of moving cells one by one (6000 rows in ~2s), and lists over 800 rows show a progress bar with Cancel (cancelling keeps every row, finishing the rest in original order).
- **Same Split for Per-Provider Fallbacks**: The per-provider Fallback Priority dialog (built-in and custom providers) has the same side-by-side two-list layout behind a draggable splitter — **Enabled priority** (first row is the Active model) and **Disabled / available**, each with its own ▲ Up / ▼ Down buttons, and draggable column headers throughout.
- **Deleted Providers Leave No Orphans**: Opening Advanced Fallback Priority now skips rows belonging to deleted custom providers (with a count tooltip); they are dropped for good on OK instead of lingering until Restore Defaults. Disabled rows also survive OK in both fallback windows (both lists are written back, not just Enabled).
- **No Shipped Model Names**: `addon/config.json` no longer ships `models` / `model_fallbacks` defaults (the code constants were already empty) — fresh installs start blank and Fetch fills them in. Existing installs keep their saved values (config migration never drops on-disk keys). Unchecking moves a row to Disabled (unchecking Active promotes the next enabled row); the last enabled model cannot be disabled. Fetched models land in Disabled; removals cannot empty the Enabled list.
- **Header Sorting in Per-Provider Fallbacks**: The per-provider Fallback Priority dialog (built-in and custom providers alike) now sorts by clicking the **Model Name**, **Thinking Level**, or **Timeout (s)** headers too (again to reverse). Thinking levels sort `off < low < medium < high`; per-model thinking/timeout values, check states, and selection follow their rows.
- **One Shared Fallback Window**: The per-provider, custom-provider, and Advanced Global fallback dialogs now run on a single `FallbackPriorityDialog` implementation (same table, search, header sorting, rank/group/sort, and row mechanics) instead of two divergent copies.
- **Per-Row Thinking/Timeout in Advanced Global List**: Global rows have their own **Thinking Level** and **Timeout (s)** columns (new `global_thinking_levels` / `global_model_timeouts` config keys, seeded from the per-provider values). They override the per-provider values for that row at runtime — including an explicit `off` — and are restored afterward, so other flows are unaffected.
...
- **Skipped Card Retry**: Cards that were skipped due to missing cloze content are now eligible for regeneration once the content is present, instead of being treated as already-generated.
- **Horizontal Log Scrolling**: The Logs tab now preserves long lines and allows horizontal scrolling instead of wrapping them.
...
- **Corrupt-Output (U+FFFD) Detection**: Generated hints/options containing `U+FFFD` replacement characters are now detected and discarded with a warning, and the fallback loop retries the next model instead of writing garbage to cards. This guards against broken provider-side model/tokenizer output (e.g. some Ollama cloud models mixing scripts), independent of the add-on's always-clean UTF-8 transport.
- **Test-Suite Determinism**: The unit suite no longer hangs on a full `discover` run (the network-monitor thread now parks on an event instead of colliding with mocked `time.sleep`) and the shared `aqt`/`mw`/`logger` mocks are isolated/restored between test modules so the whole suite runs green (230 tests).
...
- **No Hardcoded Model Names**: `DEFAULT_MODELS`, `MODEL_SUGGESTIONS`, `MODEL_FALLBACKS`, and `LEGACY_MODEL_REPLACEMENTS` are now **empty** — the add-on ships no default active model, dropdown suggestions, or fallback chains for built-in providers, since providers change their model lists frequently and any pre-shipped names only go stale and produce 404s. Set each provider's model via **Fetch Models** or by typing it in the **AI Providers** tab (persisted in `config["models"]`) and configure fallback chains in the global list. Existing installs keep their saved models; legacy names pass through and are still caught by the providers' own API deprecation flags and the "deprecated"/"legacy" substring checks.
...
- **`Test All` Now Tests Each Model Individually**: `generate_options` was called without `override_model`, so every row in a fallback test re-ran the provider's entire enabled fallback chain (active model first) instead of the specific row's model — making the run look stuck on the active model. Both the per-provider and global test loops now pass `override_model`, so each entry is validated against its own model.
- **Fetched Model Lists Drop Non-Chat Models**: `fetch_models` now filters out embeddings, OCR, moderation, TTS/transcription, realtime-audio and experimental `labs` models (e.g. `mistral-embed`, `mistral-ocr-*`, `voxtral-mini-tts-*`, `labs-leanstral-*`), so provider fallback lists no longer collect models that always fail the chat-completion test.
