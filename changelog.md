...
## Unreleased
- **Alt+Click Picker Shows Disabled Models & Providers**: The "Generate with a specific model" dialog now lists **all** providers (including disabled/unready ones, marked ⛔) and both checked *and* unchecked fallback models — disabled entries render dimmed under a separate group so explicit generation with any model/provider is always possible.
